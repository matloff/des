...
newsim <- function(simtimelim,dbg=FALSE,
      plotftn=NULL,plotmaxht=NULL,plotintrvl=NULL) {
   simlist <- new.env()
   simlist$currtime <- 0.0  # current simulated time
   simlist$simtimelim <- simtimelim
   simlist$evnts <- NULL  # event set
   simlist$dbg <- dbg
   simlist$plotftn <- plotftn
   if (!is.null(plotftn)) {
      simlist$plotintrvl <- plotintrvl
      simlist$coord <- c(0,0)
      plot(c(0,simtimelim),c(0,plotmaxht))
   }
   simlist
}
...
mainloop <- function(simlist) {
   nev <- 1
   while(simlist$currtime < simlist$simtimelim) {
      ...
      nev <- nev + 1
      if (!is.null(simlist$plotftn)) {
         if (nev %% simlist$plotintrvl) {
            oldx <- simlist$coord[1]
            oldy <- simlist$coord[2]
            newx <- simlist$currtime
            newy <- simlist$plotftn()
            lines(c(oldx,newx),c(oldy,newy))
            simlist$coord <- c(newx,newy)
         }
      }
      if (simlist$dbg) {
